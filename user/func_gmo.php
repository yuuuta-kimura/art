...
function gmo_entry($orderid,$Jobcd,$amount,$tax) {
...
//	$url = 'https://p01.mul-pay.jp/payment/EntryTran.idPass';
		...
		//'ShopID' => 'tshop00023932',
		//'ShopPass' => '4xtfv2w3',

	$data = array(
		'ShopID' => 'tshop00027241',
		'ShopPass' => 'cbrb6swk',
		'OrderID' => $orderid,
		'JobCd' => $Jobcd,
		'ItemCode' => '0000990',
	...
	"Content-Length: ".strlen('tshop00027241')+strlen('cbrb6swk')+strlen($orderid)+strlen($Jobcd)+strlen('0000990')+strlen($amount)
	+strlen($tax)
	);
	...
	//$contents='AccessID=e72ea1fed959bde46351ea2d51413875&AccessPass=c8cc0cf9f4740b32f628d2cf772af7dc';
	
	$funcret  =array();
	$retEQ = array();
	$RETGMO = array();
	$RETGMO =explode('&',$contents);
	$AccessID="";
	$AccessPass="";
	$ErrCode="";
	$ErrInfo="";
	...
	foreach ($RETGMO as &$retAND){
		$retEQ =explode('=',$retAND);
		if($retEQ[0]=='AccessID'){$AccessID=$retEQ[1];}
		if($retEQ[0]=='AccessPass'){$AccessPass=$retEQ[1];}
		if($retEQ[0]=='ErrCode'){$ErrCode=$retEQ[1];}
		if($retEQ[0]=='ErrInfo'){$ErrInfo=$retEQ[1];}
	}
	unset($retAND);
	
	if(!empty($ErrCode)){
		return array("", $ErrCode,$ErrInfo);
	}else{
		return array($AccessID, $AccessPass,"");
	}
}
...
function gmo_exec($accessid,$accesspass,$orderid,$method,$paytimes,$cardno,$expire,$securitycode) {

	$url = 'https://pt01.mul-pay.jp/payment/ExecTran.idPass';
	//$url = 'https://p01.mul-pay.jp/payment/ExecTran.idPass';
	
	//$username=mb_convert_encoding($username, "SJIS", "UTF-8");
	//$title=mb_convert_encoding(mb_substr($title,0,30), "SJIS", "UTF-8");
	$data = array(
		'AccessID' => $accessid,
	...
	$header = array(
	"Content-Type: application/x-www-form-urlencoded",
	"Content-Length: ".strlen($accessid)+strlen($accesspass)+strlen($orderid)+strlen($method)+strlen($paytimes)+strlen($cardno)
	+strlen($expire)+strlen($securitycode)+strlen('0')+strlen('0')
	);
	...
	foreach ($RETGMO as &$retAND){
		$retEQ =explode('=',$retAND);
		if($retEQ[0]=='Forward'){$Forward=$retEQ[1];}
		if($retEQ[0]=='Approve'){$Approve=$retEQ[1];}
		if($retEQ[0]=='TranID'){$TranID=$retEQ[1];}
		if($retEQ[0]=='TranDate'){$TranDate=$retEQ[1];}
		if($retEQ[0]=='CheckString'){$CheckString=$retEQ[1];}
		if($retEQ[0]=='ErrCode'){$ErrCode=$retEQ[1];}
		if($retEQ[0]=='ErrInfo'){$ErrInfo=$retEQ[1];}
	}
	unset($retAND);
	...
	if(!empty($ErrCode)){
		...
		return array($Forward, $Approve,$TranID,$TranDate,$CheckString);
	}
...
////////////////////////////////////
// 決済変更
function gmo_void($accessid,$accesspass,$jobcd) {
	...
	if(!empty($ErrCode)){
		...
		return array($Forward, $Approve,$TranID,$TranDate);
	}
}
...
function gmo_sales($accessid,$accesspass,$amount) {
...
function gmo_get_error($errorstring) {

	$errors = array();
	$errors =explode('|',$errorstring);

$CODE = array(
		'42C010000' => '42C010000:決済処理に失敗しました。申し訳ございませんが、しばらく時間をあけて購入画面からやり直してください。',
		'42C030000' => '42C030000:決済処理に失敗しました。申し訳ございませんが、しばらく時間をあけて購入画面からやり直してください。',
		'42C120000' => '42C120000:決済処理に失敗しました。申し訳ございませんが、しばらく時間をあけて購入画面からやり直してください。',
		'42C130000' => '42C130000:決済処理に失敗しました。申し訳ございませんが、しばらく時間をあけて購入画面からやり直してください。',
		'42C140000' => '42C140000:決済処理に失敗しました。申し訳ございませんが、しばらく時間をあけて購入画面からやり直してください。',
		'42C150000' => '42C150000:決済処理に失敗しました。申し訳ございませんが、しばらく時間をあけて購入画面からやり直してください。',
		'42C500000' => '42C500000:決済処理に失敗しました。申し訳ございませんが、しばらく時間をあけて購入画面からやり直してください。',
		'42C510000' => '42C510000:決済処理に失敗しました。申し訳ございませんが、しばらく時間をあけて購入画面からやり直してください。',
		'42C530000' => '42C530000:決済処理に失敗しました。申し訳ございませんが、しばらく時間をあけて購入画面からやり直してください。',
		'42C540000' => '42C540000:決済処理に失敗しました。申し訳ございませんが、しばらく時間をあけて購入画面からやり直してください。',
		'42C550000' => '42C550000:決済処理に失敗しました。申し訳ございませんが、しばらく時間をあけて購入画面からやり直してください。',
		'42C560000' => '42C560000:決済処理に失敗しました。申し訳ございませんが、しばらく時間をあけて購入画面からやり直してください。',
		'42C570000' => '42C570000:決済処理に失敗しました。申し訳ございませんが、しばらく時間をあけて購入画面からやり直してください。',
		'42C580000' => '42C580000:決済処理に失敗しました。申し訳ございませんが、しばらく時間をあけて購入画面からやり直してください。',
		'42C600000' => '42C600000:決済処理に失敗しました。申し訳ございませんが、しばらく時間をあけて購入画面からやり直してください。',
		'42C700000' => '42C700000:決済処理に失敗しました。申し訳ございませんが、しばらく時間をあけて購入画面からやり直してください。',
		'42C710000' => '42C710000:決済処理に失敗しました。申し訳ございませんが、しばらく時間をあけて購入画面からやり直してください。',
		'42C720000' => '42C720000:決済処理に失敗しました。申し訳ございませんが、しばらく時間をあけて購入画面からやり直してください。',
		'42C730000' => '42C730000:決済処理に失敗しました。申し訳ございませんが、しばらく時間をあけて購入画面からやり直してください。',
		'42C740000' => '42C740000:決済処理に失敗しました。申し訳ございませんが、しばらく時間をあけて購入画面からやり直してください。',
		'42C750000' => '42C750000:決済処理に失敗しました。申し訳ございませんが、しばらく時間をあけて購入画面からやり直してください。',
		'42C760000' => '42C760000:決済処理に失敗しました。申し訳ございませんが、しばらく時間をあけて購入画面からやり直してください。',
		'42C770000' => '42C770000:決済処理に失敗しました。申し訳ございませんが、しばらく時間をあけて購入画面からやり直してください。',
		'42C780000' => '42C780000:決済処理に失敗しました。申し訳ございませんが、しばらく時間をあけて購入画面からやり直してください。',
		'E00000000' => '特になし',
		'E01010001' => 'ショップIDが指定されていません。',
		'E01010008' => 'ショップIDに半角英数字以外の 文字が含まれているか、13文字を 超えています。',
		'E01010010' => 'ショップIDが一致しません。 ',
		'E01020001' => 'ショップパスワードが指定されていません。',
		'E01020008' => 'ショップパスワードに半角英数字 以外の文字が含まれているか、10 文字を超えています。',
		...
		'E11010017' => '180日超えの取引のため、処理を行う事が出来ません。',
		...
		'E92000002' => '只今、大変込み合っていますのでしばらく時間をあけて再度決済を行ってください。',
